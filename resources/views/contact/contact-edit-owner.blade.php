...
<h3 class="pageTitle">Edit Contact Owner</h3>



<div class="container rowHighlight">
  <form id="editContactForm" method="post" action="/contact/{{ $record->id }}/saveOwner" accept-charset="UTF-8" role="form" class="form-horizontal">
    <input type="hidden" name="_token" value="{{ csrf_token() }}">
    <input name="_method" type="hidden" value="PATCH">

    <div class="row">
      <label for="name" class="control-label text-nowrap col-xs-6 col-sm-6">&nbsp;Transfer this record:</label>
      <span class="form-control-static col-xs-6 col-sm-3">{{ \App\AppHelper::optionValueFromDB('lkp_salutation', $record->salutation_id) }} {{ $record->first_name }} {{ $record->last_name }}</span>
    </div>
